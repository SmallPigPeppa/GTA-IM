...
def get_scene_render(info_npz,body_mesh,
    ...
    word2cam=np.array(info_npz['world2cam_trans'])
    # camera_pose = np.hstack([word2cam[0][:3,:3], word2cam[0][3,:3]])
    print(word2cam[0])
    camera_pose = np.vstack([word2cam[0], [0, 0, 0, 1]])
...
def vis_skeleton_pcd(rec_idx, f_id, fusion_window=20):
    ...
    o3d.visualization.draw_geometries([global_pcd,mesh])
